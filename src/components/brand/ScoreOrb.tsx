...
export function ScoreOrb({
  ...
  return (
    ...
    >
      <div className="score-ticks absolute -inset-[7%] rounded-full opacity-55" aria-hidden />
      <div className="score-ring absolute inset-0 rounded-full shadow-[0_30px_80px_rgb(0,82,255,0.22)]" />
      <div className="relative z-10 flex flex-col items-center text-center">
        {score == null ? (
          <>
            ...
            <p className="mt-3 font-heading text-lg font-normal tracking-tight sm:text-xl">CrownScore</p>
            <p className="mt-1 text-xs font-bold text-muted-foreground">{label}</p>
          </>
        ) : (
          <>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-primary">{label}</p>
