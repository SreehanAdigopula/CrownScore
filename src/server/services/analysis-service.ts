...
export const analysisInputSchema = z.object({
  ...
  baselineRatio: z.number().positive().max(1).default(0.45),
  previousScore: z.number().positive().nullable().default(null),
...
export async function analyzeCheckIn(rawInput: unknown) {
  ...
  const previousChangePercent = input.previousScore == null ? null : percentChange(normalizedScore, input.previousScore);
