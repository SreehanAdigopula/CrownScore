...
export const analysisInputSchema = z.object({
  ...
  questionnaire: z
    .object({
      adherenceRate: z.number().min(0).max(100).nullable().default(null),
      shedding: z.enum(["LOW", "TYPICAL", "HIGH"]).default("TYPICAL"),
      irritation: z.boolean().default(false),
      scalpPain: z.boolean().default(false),
      routineChanged: z.boolean().default(false),
      notes: z.string().max(600).optional(),
    })
    .default({ adherenceRate: null, shedding: "TYPICAL", irritation: false, scalpPain: false, routineChanged: false }),
});
...
export async function analyzeCheckIn(rawInput: unknown) {
  ...
  const safety = evaluateSafety({
    concerns: score.concerns.map((item) => item.className),
    qualityStatus: input.quality.status,
    questionnaire: input.questionnaire as Questionnaire,
  });
  const analysis: CheckInAnalysis = {
    id: crypto.randomUUID(),
    capturedAt: new Date().toISOString(),
    treatmentWeek: input.treatmentWeek,
    ...
    adherenceRate: input.questionnaire.adherenceRate,
    isDemoData: false,
  ...
  const coachInput = {
    ...
    concernLabels: analysis.concerns.map((item) => item.label),
    status: analysis.status,
    safetyStatus: analysis.safetyStatus,
    adherenceRate: analysis.adherenceRate,
    coachStyle: input.coachStyle as CoachStyle,
  };
  let coach;
  try {
    ...
    coach = await fallback.generateSummary(coachInput);
  }
